...
class LancementBD:
    ...
    def __init__(self):
        ...
        self.scripts = ["destructionBD.sql",
                        "creationBD.sql",
                        "triggersBD.sql",
                        "insertionBD.sql"]
    ...
    def main(self):
        ...
        if LOGIN == "" or PASSWD == "" or SERVEUR == "" or BD == "":
            print("Erreur : Il manque des informations dans le fichier config.py")
            sys.exit(1)
        ...
        for script in self.scripts:
            ...
            if not os.path.exists(script_path):
                print(f"Attention : Fichier '{script_path}' introuvable. Ignoré.")
                sys.exit(1)
            print(f"Exécution de {script}...")
            try:
                with open(script_path, 'r') as f:
                    subprocess.run(
                        self.commande_mysql + [BD],
                        stdin=f,
                        env=self.env,
                        check=True
                    )
