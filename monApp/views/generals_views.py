...
def index():
    les_derniers_articles = PresseBD.query.order_by(
        PresseBD.dateP.desc()).limit(3).all()
    les_dernieres_informations = InformationBD.query.order_by(
        InformationBD.dateIN.desc()).limit(3).all()
    les_dernieres_competitions = CompetitionBD.query.filter_by(
        passee=True).order_by(CompetitionBD.date_debut.desc()).limit(3).all()
    return render_template("generals/index.html",
                           title=TITLE,
                           articles=les_derniers_articles,
                           informations=les_dernieres_informations,
                           competitions=les_dernieres_competitions)
...
def horaires():
    ...
    return render_template("generals/horaire.html",
                           title=TITLE + "- Horaires",
                           horaires=les_horaires)


@general_bp.route("/adhesions/")
def adhesions():
    tarifs_adhesion = TarifBD.query.filter_by(categorie='Adhesion').all()
    return render_template("generals/adhesion.html",
                           title=TITLE + "- Adhésions",
                           tarifs=tarifs_adhesion)


@general_bp.route("/materiel/")
def materiel():
    tarifs_materiel = TarifBD.query.filter_by(categorie='Materiel').all()
    return render_template("generals/materiel.html",
                           title=TITLE + "- Matériel et tenues",
                           tarifs=tarifs_materiel)
...
def escrime_feminin():
    return render_template("generals/escrime_feminin.html",
                           title=TITLE + "- L'escrime Féminin")
...
@general_bp.route("/contact/", methods=(
    "GET",
    "POST",
))
def contact():
    ...
    if form.validate_on_submit():
        ...
        nouveau_formulaire = FormulaireBD(type=type_f,
                                          sujet=sujet,
                                          email=email,
...
def historique():
    return render_template("generals/historique.html",
...
def comite_cercle():
    comite = {
        "president":
            MembreBD.query.filter_by(statut='Président').first(),
        "vicePresident":
            MembreBD.query.filter_by(statut='Vice-Président').first(),
        "tresorier":
            MembreBD.query.filter_by(statut='Trésorier Général').first(),
        "secretaire":
            MembreBD.query.filter_by(statut='Secrétaire Générale').first()
    }
    membres = MembreBD.query.filter_by(statut='Membre du Comité').all()
    return render_template("generals/comite_cercle.html",
                           title=TITLE + "- Comité directeur du Cercle",
                           comite=comite,
                           membres=membres)
