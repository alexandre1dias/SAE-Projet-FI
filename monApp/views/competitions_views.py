...
def competitions(etat="prochaines"):
    ...
    if annee_selectionnee:
        try:
            ...
            lesCompetitions = lesCompetitions.filter(
                CompetitionBD.date_debut >= debut_saison,
                CompetitionBD.date_debut <= fin_saison)
        except (ValueError, TypeError):
            pass
    filtre.tri.choices = [('date_desc', 'Plus récent'),
                          ('date_asc', 'Plus ancien')]
    if filtre.tri.data == "date_asc":
        lesCompetitions = lesCompetitions.order_by(
            CompetitionBD.date_debut.asc())
    else:
        lesCompetitions = lesCompetitions.order_by(
            CompetitionBD.date_debut.desc())

    lesCompetitions = lesCompetitions.filter(CompetitionBD.passee == passee)

    if filtre.sexe.data:
        lesCompetitions = lesCompetitions.filter(
            CompetitionBD.sexe.in_(filtre.sexe.data))
    if filtre.niveau.data:
        lesCompetitions = lesCompetitions.filter(
            or_(*(CompetitionBD.niveaux.like(f"%{n}%")
                  for n in filtre.niveau.data)))
    ...
    pagination = lesCompetitions.paginate(page=page,
                                          per_page=6,
                                          error_out=False)
    return render_template("competitions/competitions.html",
...
def competition_view(idCompetition):
    ...
    if current_user.is_authenticated and session.get('user_type') == 'membre':
        participation = ParticiperBD.query.filter_by(
            ...
            id_event=uneCompetition.id_event).first()
    ...
    classement_pdf_path = os.path.join(app.root_path, 'static', 'classements',
                                       str(uneCompetition.id), 'classement.pdf')
    classement_pdf_exists = os.path.exists(classement_pdf_path)
    return render_template("competitions/competition_view.html",
                           title=TITLE + "- Consultation de la competition",
                           competition=uneCompetition,
                           origine=origine,
                           deja_inscrit=deja_inscrit,
                           est_eligible=est_eligible,
                           lesResultats=resultats,
                           classement_pdf_exists=classement_pdf_exists)


@competitions_bp.route("/inscrire/competition/<int:idCompetition>",
                       methods=['GET'])
@login_required
@membre_required
def inscrire_competition(idCompetition):
    ...
    try:
        ...
        db.session.rollback()
    return redirect(
        url_for('competitions.competition_view', idCompetition=idCompetition))


@competitions_bp.route("/desinscrire/competition/<int:idCompetition>",
                       methods=['GET'])
@login_required
def desinscrire_competition(idCompetition):
    competition_obj = CompetitionBD.query.get_or_404(idCompetition)
    participation = ParticiperBD.query.filter_by(
        id_membre=current_user.id, id_event=competition_obj.id_event).first()
    if participation:
        try:
...
@competitions_bp.route("/competition_update/<int:idCompetition>",
                       methods=['GET', 'POST'])
@login_required
@admin_required
def competition_update(idCompetition):
    ...
    if request.method == 'POST':
        try:
            ...
            competition.date_debut = datetime.strptime(
                request.form['date_debut'], '%Y-%m-%d').date()
            competition.heure_debut = request.form['heure_debut']
            competition.date_fin = datetime.strptime(request.form['date_fin'],
                                                     '%Y-%m-%d').date()
            ...
            db.session.commit()
            return redirect(
                url_for('competitions.competition_view',
    ...
    participations = ParticiperBD.query.filter_by(
        id_event=competition.id_event).all()
    participants = [p.membre for p in participations]

    return render_template("competitions/competition_update.html",
                           title=TITLE + "- Modification de la competition",
                           competition=competition,
                           lesParticipants=participants,
                           origine=origine)


@competitions_bp.route("/competition/<int:idC>/inscrire_membres",
                       methods=['GET'])
@login_required
@admin_required
def inscrire_membres_competition(idC):
    ...
    tous_les_non_participants = MembreBD.query.filter(
    ...
    niveaux_liste = [
        niveau.strip() for niveau in competition.niveaux.split(',')
    ]
    ...
    return render_template("competitions/competition_inscrire_membre.html",
                           title=TITLE + "- Inscrire des membres",
                           competition=competition,
                           non_participants=non_participants_eligibles,
                           membres_a_inscrire_ids=membres_a_inscrire_ids)


@competitions_bp.route("/competition/<int:idC>/inscription_membres",
                       methods=['POST'])
@login_required
@admin_required
def inscription_membres_competition(idC):
    ...
    for membre_id in membres_a_inscrire_ids:
        deja_inscrit = ParticiperBD.query.filter_by(
            id_membre=membre_id, id_event=competition.id_event).first()
        if not deja_inscrit:
            nouvelle_participation = ParticiperBD(id_membre=membre_id,
                                                  id_event=competition.id_event)
            db.session.add(nouvelle_participation)
    db.session.commit()
    return redirect(
        url_for('competitions.competition_update', idCompetition=idC))


@competitions_bp.route(
    "/competition/<int:idCompetition>/classer/<int:idMembre>", methods=['POST'])
@login_required
@admin_required
def classer_membre(idCompetition, idMembre):
    ...
    if not classement:
        return redirect(
            url_for('competitions.competition_update',
                    idCompetition=idCompetition))
    if not classement.isdigit():
        return redirect(
            url_for('competitions.competition_update',
                    idCompetition=idCompetition))
    resultat_existant = ResultatBD.query.filter_by(id_competition=idCompetition,
                                                   id_membre=idMembre).first()
    if resultat_existant:
        resultat_existant.resultat = classement
    else:
        nouveau_resultat = ResultatBD(resultat=classement,
                                      date=competition.date_fin,
                                      type_arme=competition.type_arme,
                                      type_compete=competition.typeComp,
                                      id_competition=idCompetition,
                                      id_membre=idMembre)
        db.session.add(nouveau_resultat)
    db.session.commit()
    return redirect(
        url_for('competitions.competition_update', idCompetition=idCompetition))


@competitions_bp.route("/competition/<int:idCompetition>/upload_classement",
                       methods=['POST'])
@login_required
@admin_required
def upload_classement_competition(idCompetition):
    ...
    if file and file.filename.lower().endswith('.pdf'):
        ...
        dossier_classement = os.path.join(app.root_path, 'static',
                                          'classements', str(competition.id))
        ...
        flash(
            'Type de fichier non autorisé. Veuillez téléverser un fichier PDF.',
            'danger')
    return redirect(
        url_for('competitions.competition_update', idCompetition=idCompetition))


@competitions_bp.route("/competition/<int:idC>/delete/<int:idM>",
                       methods=['POST'])
@login_required
@admin_required
def delete_membre_competition(idC, idM):
    competition = CompetitionBD.query.get_or_404(idC)
    participation = ParticiperBD.query.filter_by(id_event=competition.id_event,
...
@competitions_bp.route("/competition/<int:idCompetition>/add_image",
                       methods=['POST'])
@login_required
@admin_required
def add_image_competition(idCompetition):
    ...
    if file:
        ...
        dossier_images = os.path.join(app.root_path, 'static', 'images',
                                      'competitions', str(competition.id))
        os.makedirs(dossier_images, exist_ok=True)
        file.save(os.path.join(dossier_images, filename))

        image_url = os.path.join('images', 'competitions', str(competition.id),
                                 filename).replace('\\', '/')
        alt_text = request.form.get('alt', filename)
        prive = 'prive' in request.form

        try:
            nouvelle_image = ImageAppBD(urlI=image_url,
                                        alt=alt_text,
                                        prive=prive)
            ...
            db.session.rollback()
            flash(f'Erreur lors de l\'ajout de l\'image à la base de données.',
                  'danger')
...
@competitions_bp.route("/competition/delete_image/<int:idImage>",
                       methods=['POST'])
@login_required
@admin_required
def delete_image_competition(idImage):
    ...
    if image_a_retirer in competition.images_rc:
        ...
        flash('Cette image n\'était pas associée à cette compétition.',
              'warning')
...
@competitions_bp.route("/competition_delete/<int:idCompetition>",
                       methods=['POST'])
@login_required
@admin_required
