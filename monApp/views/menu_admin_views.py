...
def formulaire_view(idFormulaire):
    ...
    return render_template("menu_admin/formulaire_view.html",
                           title=TITLE + "- Consultation de Formulaire",
                           selectedFormulaire=unFormulaire)
...
def api_get_reponse(idFormulaire):
    formulaire = FormulaireBD.query.get_or_404(idFormulaire)
    if session.get(
            'user_type') == 'membre' and formulaire.idMembre != current_user.id:
    ...
    return jsonify({
        ...
        'reponse':
            formulaire.reponse,
        'date':
            formulaire.date.strftime('%d/%m/%Y')
            if formulaire.date else 'Date inconnue'
    })
...
def repondre_formulaire(idFormulaire):
    ...
    if leFormulaire.membre and leFormulaire.membre.reponseFormulaireSite:
        ...
        notif = NotifsBD(typeN='Réponse Formulaire',
...
def gerer_profils():
    ...
    filtre.tri.choices = [('date_desc', 'Plus récent'),
                          ('date_asc', 'Plus ancien'), ('nom_asc', 'Nom A-Z'),
                          ('nom_desc', 'Nom Z-A'), ('prenom_asc', 'Prenom A-Z'),
                          ('prenom_desc', 'Prenom Z-A'),
    ...
    if filtre.recherche.data:
        ...
        liste = liste.filter(
            or_(MembreBD.nom.ilike(terme), MembreBD.prenom.ilike(terme),
...
@admin_bp.route('/gerer_anciens_profils/reinscrire/<int:idM>',
                methods=("POST",))
@login_required
@admin_required
...
def profil_edit(idM):
    ...
    if unForm.validate_on_submit():
        ...
        if action == 'admin_save':
            ...
            for admin in admins:
                if admin.demandeModifSite:
                    notif = NotifsBD(
                        typeN='modification',
                        sourceN=
                        f"Demande de modification de {unMembre.prenom} {unMembre.nom}",
                        lue=False,
                        timestamp=datetime.now(),
                        idAdmin=admin.id,
            ...
            db.session.commit()
            return redirect(
                url_for('profil.profil_view', idM=idM, origine='profil'))
...
def accepter_inscription(idI):
    ...
    nouveauMembre = MembreBD(nom=inscription.nom,
...
def gestion_tarifs():
    ...
    if form.validate_on_submit():
        nouveau_tarif = TarifBD(nom=form.nom.data,
                                prix=form.prix.data,
    ...
    return render_template("menu_admin/admin_gestion_tarifs.html",
...
def edit_tarif(idT):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('admin.gestion_tarifs'))
    return render_template("menu_admin/admin_edit_tarif.html",
...
def gestion_horaires():
    form = HoraireForm()
    if form.validate_on_submit():
        nouveau_horaire = HoraireBD(jour=form.jour.data,
    ...
    return render_template("menu_admin/admin_gestion_horaires.html",
...
def edit_horaire(idH):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('admin.gestion_horaires'))
    return render_template("menu_admin/admin_edit_horaire.html",
