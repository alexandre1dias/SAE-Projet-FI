...
class ModifForm(FlaskForm):
    ...
    sexe = SelectField('Sexe concerné',
                       choices=[('Homme', 'Homme'), ('Femme', 'Femme')],
                       validators=[DataRequired()])
    email = StringField('Email', validators=[DataRequired(), Email()])
    numTel = StringField('Numéro de téléphone (optionnel)',
                         validators=[Optional()])
    ...
    statut = SelectField('Statut',
                         choices=[('Membre', 'Membre'),
                                  ...
                                  ('Président', 'Président')],
                         validators=[DataRequired()])
    justification = TextAreaField('Justification de la demande (optionnel)',
                                  validators=[Optional()])
...
class InscriptionForm(FlaskForm):
    ...
    date_naissance = DateField('date de naissance',
                               format='%Y-%m-%d',
                               validators=[DataRequired(), validate_age])
...
class ContactForm(FlaskForm):
    type_form = RadioField('Type de formulaire',
...
class EventForm(FlaskForm):
    ...
    start_date = DateTimeLocalField('Date et heure de début',
                                    format='%Y-%m-%dT%H:%M',
                                    validators=[DataRequired()])
    end_date = DateTimeLocalField('Date et heure de fin',
                                  format='%Y-%m-%dT%H:%M',
                                  validators=[DataRequired()])
    category = SelectField('Catégorie',
                           choices=[('Entraînement', 'Entraînement'),
                                    ('Compétition', 'Compétition'),
                                    ('Réunion', 'Réunion'),
                                    ('Evenement du club', 'Evenement du club')],
                           validators=[DataRequired()])
    level = SelectMultipleField('Niveaux concernés',
                                choices=[('M9', 'M9'), ('M11', 'M11'),
                                         ('M13', 'M13'), ('M15', 'M15'),
                                         ('M17', 'M17'), ('M20', 'M20'),
                                         ('Senior', 'Senior'),
                                         ('Veteran', 'Veteran')],
    ...
    arme = SelectField('Arme concernée',
                       choices=[('Fleuret', 'Fleuret'), ('Épée', 'Épée'),
                                ('Sabre', 'Sabre')],
                       validators=[])

    type = SelectField('Type d\'événement',
                       choices=[('Regionale', 'Regionale'),
                                ('National', 'National')],
                       validators=[])
    ...
    def validate_level(self, level):
        ...
        if selected_category in [
                'Compétition', 'Entraînement', 'Evenement du club'
        ] and not selected_levels_data:
            raise ValidationError(
                'Le niveau est requis pour ce type d\'événement.')
        if selected_category == 'Compétition' and selected_levels_data:
            selected_levels = set(selected_levels_data)
            consecutive_pairs = [{'M9', 'M11'}, {'M11', 'M13'}, {'M13', 'M15'},
                                 {'M15', 'M17'}, {'M17', 'M20'},
                                 {'M20', 'Senior'}, {'Senior', 'Vétéran'}]
            for pair in consecutive_pairs:
                if selected_levels.issuperset(pair):
                    friendly_pair = " et ".join(sorted(list(pair)))
                    raise ValidationError(
                        f'Règle de surclassement : Les niveaux {friendly_pair} ne peuvent pas être sélectionnés ensemble.'
                    )
    ...
    def validate_arme(self, arme):
        if self.category.data in ['Compétition', 'Entraînement'
                                 ] and not arme.data:
            raise ValidationError(
                'L\'arme est requise pour ce type d\'événement.')

    def validate_type(self, type):
        if self.category.data == 'Compétition' and not type.data:
            raise ValidationError(
                'Le type (Régional/National) est requis for une compétition.')

    def validate_ville(self, ville):
        if self.category.data in ['Compétition', 'Evenement du club'
                                 ] and not ville.data:
            raise ValidationError(
                'La ville est requise pour ce type d\'événement.')

    def validate_adresse(self, adresse):
        if self.category.data in ['Compétition', 'Evenement du club'
                                 ] and not adresse.data:
            raise ValidationError(
                'L\'adresse est requise pour ce type d\'événement.')
...
class ParametresForm(FlaskForm):
    ...
    age = StringField('age',
                      validators=[Optional()],
                      render_kw={'readonly': True})
    date = StringField('nouvelle date de naissance',
                       validators=[Optional()],
                       render_kw={'readonly': True})
...
class FiltreForm(FlaskForm):
    ...
    CHOIX_NIVEAU = [('M9', 'M9'), ('M11', 'M11'), ('M13', 'M13'),
                    ('M15', 'M15'), ('M17', 'M17'), ('M20', 'M20'),
                    ('Senior', 'Senior'), ('Vétéran', 'Vétéran')]
    CHOIX_FORMULAIRE = [('Question', 'Questions'), ('Demande', 'Demandes'),
    ...
    CHOIX_TYPE_EVENT = [('Compétition', 'Compétition'), ('Réunion', 'Réunion'),
                        ('Évènement du club', 'Évènement du club'),
                        ('Entraînement', 'Entraînement')]

    sexe = SelectMultipleField('Sexes',
                               choices=CHOIX_SEXE,
                               default=[c[0] for c in CHOIX_SEXE],
                               option_widget=widgets.CheckboxInput(),
                               widget=widgets.ListWidget(prefix_label=False))

    niveau = SelectMultipleField('Niveaux',
    ...
    armes = SelectMultipleField('Armes',
                                choices=CHOIX_ARMES,
    ...
    CHOIX_TRI = [('date_desc', 'Plus récent'), ('date_asc', 'Plus ancien'),
                 ('resultat', 'Resultat'), ('nom_asc', 'Nom A-Z'),
                 ('nom_desc', 'Nom Z-A'), ('prenom_asc', 'Prenom A-Z'),
                 ('prenom_desc', 'Prenom Z-A'), ('age_asc', 'Plus agé'),
                 ('age_desc', 'Plus jeune')]
    tri = SelectField('Trier par', choices=CHOIX_TRI, default='date_desc')
    annee_scolaire = SelectField('Années', choices=[])


class HoraireForm(FlaskForm):
    jour = SelectField('Jour',
                       choices=[('Lundi', 'Lundi'), ('Mardi', 'Mardi'),
                                ('Mercredi', 'Mercredi'), ('Jeudi', 'Jeudi'),
                                ('Vendredi', 'Vendredi'), ('Samedi', 'Samedi'),
                                ('Dimanche', 'Dimanche')],
                       validators=[DataRequired()])
    heure_debut = StringField('Heure de début (ex: 19h00)',
                              validators=[DataRequired()])
    heure_fin = StringField('Heure de fin (ex: 21h15)',
                            validators=[DataRequired()])
    activite = StringField('Activité (ex: Entraînement Épée)',
                           validators=[DataRequired()])
    details = TextAreaField('Détails (ex: M17, M20...)',
                            validators=[Optional()])
    submit = SubmitField('Enregistrer')


class TarifForm(FlaskForm):
    nom = StringField('Intitulé (ex: Initiation, Location annuelle)',
                      validators=[DataRequired()])
    prix = IntegerField('Prix (€)', validators=[DataRequired()])
    description = TextAreaField('Description', validators=[Optional()])
    categorie = SelectField('Catégorie',
                            choices=[('Adhesion', 'Adhésion'),
                                     ('Materiel', 'Matériel')],
                            validators=[DataRequired()])
    submit = SubmitField('Enregistrer')
...
class ArticleForm(FlaskForm):
    ...
    contenu = TextAreaField('Contenu complet',
                            validators=[DataRequired()],
                            render_kw={"rows": 10})
    images = MultipleFileField('Ajouter des photos',
                               validators=[
...
class PresseForm(FlaskForm):
    titre = StringField('Titre de l\'article', validators=[DataRequired()])
    contenu = TextAreaField('Description / Contenu',
                            validators=[DataRequired()],
                            render_kw={"rows": 5})
