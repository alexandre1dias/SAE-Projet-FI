...
def test_new_membre(app, db):
    ...
    new_membre = MembreBD(nom='Desgranges',
                          prenom='Lucas',
...
def test_inscription_attente(app, db):
    ...
    inscr = InscriptionBD(email='nouveau@test.fr',
                          nom='Dupont',
                          prenom='Jean',
                          ddn=date(2000, 1, 1),
                          mdp_hash='pass123',
...
def test_competition(app, db):
    ...
    compet = CompetitionBD(nom='Compétition Régionale',
                           ville='Orléans',
...
def test_entrainement(app, db):
    ...
    entrainement = EntrainementBD(jour='Lundi',
                                  date=date(2024, 1, 1),
                                  id_event=event.id)
...
def test_event_club(app, db):
    ...
    evt_club = EventClubBD(NomEV='Barbecue Fin Année',
...
def test_formulaire_et_reponse(app, db):
    ...
    form = FormulaireBD(sujet='Question',
                        email='user@f.fr',
                        date=date.today(),
                        idMembre=membre.id)
    db.session.add(form)
...
def test_article_et_images(app, db):
    ...
    art = ArticleBD(titre='Championnat',
                    contenu='Super contenu',
                    date=date.today())
    db.session.add(art)
    db.session.flush()

    img = ImageArticleBD(nom='photo.jpg', id_article=art.id)
    db.session.add(img)
    db.session.commit()
...
def test_notification(app, db):
    ...
    notif = NotifsBD(typeN='modification',
                     sourceN='Test Source',
                     lue=False,
                     timestamp=ts,
...
def test_tables_liaison_notification(app, db):
    ...
    ins_a = recevoir_a.insert().values(idNotifs=notif.idNotifs,
                                       idAdmin=admin.id)
    ins_m = recevoir_m.insert().values(idNotifs=notif.idNotifs,
                                       idMembre=membre.id)
...
def test_notification_fk_users(app, db):
    ...
    notif_complete = NotifsBD(typeN="Info FK",
