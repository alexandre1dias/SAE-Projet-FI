...
def test_login_form(app):
    ...
    with app.test_request_context():
        ...
        form_bad = LoginForm(formdata=MultiDict({
            'email': 'bad',
            'password': ''
        }))
...
def test_reset_password_form(app):
    ...
    with app.test_request_context():
        ...
        assert form_bad.validate() is False
        assert 'Les mots de passe ne correspondent pas.' in form_bad.errors[
            'confirm_password']
...
def test_inscription_form_age_et_mdp(app, db):
    ...
    with app.test_request_context():
        ...
        assert form.validate() is True
        # Sauvegarde
        new_inscription = InscriptionBD(email=form.Login.data,
                                        nom=form.nom.data,
...
def test_event_form_competition_valide(app, db):
    ...
    with app.test_request_context():
        ...
        assert CompetitionBD.query.filter_by(
            nom='Grand Tournoi').first().type_arme == 'Fleuret'
...
def test_event_form_regles_specifiques(app):
    ...
    with app.test_request_context():
        ...
        assert 'La ville est requise pour ce type d\'événement.' in str(
            form.errors['ville'])
...
def test_contact_form(app, db):
    ...
    with app.test_request_context():
        ...
        assert form.validate() is True
        f = FormulaireBD(type=form.type_form.data,
...
def test_presse_form(app, db):
    ...
    with app.test_request_context():
        ...
        assert form.validate() is True
        p = PresseBD(titreP=form.titre.data,
