...
def club_view(idEventClub):
    ...
    return render_template("club_view.html",title=TITLE+"- un évenement du club",selectedEventClub=unEventClub, deja_inscrit=deja_inscrit)
...
def add_event():
    ...
    if form.validate_on_submit():
        nouvel_evenement = Event(
            ...
            level=", ".join(form.level.data) # "level" est un mot-clé, "niveau" serait mieux
        )
